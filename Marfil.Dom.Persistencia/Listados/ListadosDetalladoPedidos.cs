...
{
    public class ListadosDetalladoPedidos : ListadosModel
    {
        private List<string> _series = new List<string>();

        #region Properties

        public override string TituloListado => "Listado detallado de Pedidos";

        public override string IdListado => FListadosModel.PedidosDetallado;
        ...
        public ListadosDetalladoPedidos()
        {

        }

        public ListadosDetalladoPedidos(IContextService context) : base(context)
        {
        ...
        internal override string GenerarSelect()
        {
            var sb = new StringBuilder();
                ...
                " pl.fkarticulos as [Cod. Artículo], pl.descripcion as [Artículo],ud.textocorto as [UM],cn.descripcion as [Canal], pl.cantidad as [Cantidad],pl.Largo as [Largo],pl.Ancho as [Ancho],pl.Grueso as [Grueso],pl.metros as [Metros], pl.precio as [Precio],pl.porcentajedescuento as [% Dto.],pl.importe as Subtotal,mo.abreviatura as [Moneda],mo.decimales as [_decimales], ud.decimalestotales as [_decimalesunidades] from Pedidos as p " +
                " inner join Pedidoslin as pl on pl.empresa= p.empresa and pl.fkpedidos= p.id inner join clientes as c on c.empresa= p.empresa and p.fkclientes= c.fkcuentas inner join cuentas as cu on c.empresa= cu.empresa and c.fkcuentas=cu.id and cu.tipocuenta= " +
