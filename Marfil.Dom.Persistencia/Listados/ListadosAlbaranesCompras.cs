...
{
    public class ListadosAlbaranesCompras : ListadosModel
    {
        private List<string> _series = new List<string>();

        #region Properties

        public override string TituloListado => "Listado albaranes de compra";

        public override string IdListado
        {
            get { return Agrupacion == "2" ? FListadosModel.AlbaranesComprasCabeceraAgrupado : FListadosModel.AlbaranesComprasCabecera; }
        }

        public override string WebIdListado => FListadosModel.AlbaranesComprasCabecera;
        ...
        public ListadosAlbaranesCompras()
        {
            
        }

        public ListadosAlbaranesCompras(IContextService context):base(context)
        {
        ...
        internal override string GenerarFiltrosColumnas()
        {
            ...
            if (!string.IsNullOrEmpty(Fkfamiliacliente))
            {
                ...
                ValoresParametros.Add("fkfamiliaproveedor", Fkfamiliacliente);
                sb.Append(" c.fkfamiliaproveedor=@fkfamiliaproveedor ");
                Condiciones.Add(string.Format("{0}: {1}", RProveedor.Fkfamiliaproveedor, Fkfamiliacliente));
                flag = true;
            }
            ...
            if (!string.IsNullOrEmpty(Fkzonacliente))
            {
                ...
                ValoresParametros.Add("fkzonaproveedor", Fkzonacliente);
                sb.Append(" c.fkzonaproveedor=@fkzonaproveedor ");
                Condiciones.Add(string.Format("{0}: {1}", RProveedor.Fkzonaproveedor, Fkzonacliente));
                flag = true;
            }
            ...
            if (!string.IsNullOrEmpty(Fkmonedas))
            {
                ...
                Condiciones.Add(string.Format("{0}: {1}", RAlbaranes.Fkmonedas, serviceMonedas.getAll().Select(f=>(MonedasModel)f).SingleOrDefault(f => f.Id ==int.Parse(Fkmonedas)).Descripcion));
                flag = true;
            }
        ...
        internal override string GenerarSelect()
        {
            ...
            if (Agrupacion == "1")
            {
                ...
                " left join formaspago as fp on fp.id=isnull(c.fkformaspago,ac.fkformaspago) " +
                ...
                " left join zonasclientes as zc on zc.valor= isnull(c.fkzonaproveedor,ac.fkzonaacreedor) " +
