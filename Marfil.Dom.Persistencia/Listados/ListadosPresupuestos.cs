...
namespace Marfil.Dom.Persistencia.Listados
{
    ...
    public class ListadosPresupuestos : ListadosModel
    {
        private List<string> _series = new List<string>();

        #region Properties

        public override string TituloListado => "Listado de Presupuestos";

        public override string IdListado
        {
            get { return Agrupacion=="2" ? FListadosModel.PresupuestosCabeceraAgrupado : FListadosModel.PresupuestosCabecera; }
        }

        public override string WebIdListado => FListadosModel.PresupuestosCabecera;
        ...
        public ListadosPresupuestos()
        {

        }

        public ListadosPresupuestos(IContextService context) : base(context)
        {
        ...
        internal override string GenerarFiltrosColumnas()
        {
            ...
            if (!string.IsNullOrEmpty(Estado) && !Estado.Equals("0-"))
            {
                ...
                Condiciones.Add(string.Format("{0}: {1}", RPresupuestos.Estado, Estados.First(f=>f.CampoId==Estado).Descripcion));
                flag = true;
            }
            ...
            if (!string.IsNullOrEmpty(Fkmonedas))
            {
                ...
                Condiciones.Add(string.Format("{0}: {1}", RPresupuestos.Fkmonedas, serviceMonedas.getAll().Select(f=>(MonedasModel)f).SingleOrDefault(f => f.Id ==int.Parse(Fkmonedas)).Descripcion));
                flag = true;
            }
