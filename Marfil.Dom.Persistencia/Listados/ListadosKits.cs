...
{
    public class ListadosKits : ListadosModel
    {
      

        #region Properties

        public override string TituloListado => "Listado de Kits";

        public override string IdListado => FListadosModel.ListadoKits;
        ...
        public ListadosKits(IContextService context):base(context)
        {
            Agrupacion = "1";
        ...
        internal override string GenerarSelect()
        {
            ...
            {
                sb.AppendFormat(
                "select  ud.decimalestotales as [_decimalesunidades],p.referencia [Kit], a.id as [Cod. Artículo],a.Descripcionabreviada as  [Artículo],az.descripcion as [Zona Almacen],sum(pl.cantidad) as [Cantidad],gm.descripcion as [Grupo material],fm.descripcion as [Familia material],sum(pl.metros) as [Metros],ud.textocorto as [UM], sum(sh.pesonetolote) as [Peso Neto] from kit as p " +
                " inner join kitlin as pl on pl.empresa= p.empresa and pl.fkkit = p.id " +
                ...
                " left join almaceneszona as az on az.empresa=p.empresa and az.fkalmacenes= p.fkalmacen and az.id=p.fkzonalamacen " +
            ...
            {
                sb.AppendFormat(
                 "select  ud.decimalestotales as [_decimalesunidades],p.referencia [Kit], a.id as [Cod. Artículo],a.Descripcionabreviada as  [Artículo],az.descripcion as [Zona Almacen],sum(pl.cantidad) as [Cantidad],gm.descripcion as [Grupo material],fm.descripcion as [Familia material],pl.largo as [Largo],pl.ancho as [Ancho],pl.grueso as [Grueso],pl.metros as [Metros],ud.textocorto as [UM] from kit as p " +
                 " inner join kitlin as pl on pl.empresa= p.empresa and pl.fkkit = p.id  " +
                 " inner join articulos as a on a.empresa=p.empresa and  a.id=pl.fkarticulos and isnull(a.articulocomentario,0)=0 " +
                 " inner join familiasproductos as fp on fp.empresa= p.empresa and fp.id=substring(pl.fkarticulos,0,3) " +
                 " left join materiales  as m on m.empresa=p.empresa and m.id=substring(pl.fkarticulos,3,3) " +
        ...
        internal override string GenerarOrdenColumnas()
        {
            ...
            {
                case "1":
                    result = " group by  p.referencia,a.id,a.descripcionabreviada,az.descripcion,gm.descripcion,fm.descripcion,ud.textocorto,  ud.decimalestotales";
                    break;
                case "2":
                    result = "  group by p.referencia, a.id,a.descripcionabreviada,az.descripcion,gm.descripcion,fm.descripcion,ud.textocorto, ud.decimalestotales,pl.largo,pl.ancho,pl.grueso,pl.metros";
                    break;
            }


            result += " order by p.referencia,a.id";
