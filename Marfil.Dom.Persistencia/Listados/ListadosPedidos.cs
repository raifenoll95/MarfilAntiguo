...
{
    public class ListadosPedidos : ListadosModel
    {
        private List<string> _series = new List<string>();

        #region Properties

        public override string TituloListado => "Listado de Pedidos";

        public override string IdListado
        {
            get { return Agrupacion == "2" ? FListadosModel.PedidosCabeceraAgrupado : FListadosModel.PedidosCabecera; }
        }

        public override string WebIdListado => FListadosModel.PedidosCabecera;
        ...
        public ListadosPedidos()
        {

        }

        public ListadosPedidos(IContextService context) : base(context)
        {
        ...
        internal override string GenerarFiltrosColumnas()
        {
            ...
            if (Series?.Any() ?? false)
            {
                ...
                Condiciones.Add(string.Format("{0}: {1}", RPedidos.SeriesListado, string.Join(", ", Series.ToArray())));
                flag = true;
            }
            ...
            if (!string.IsNullOrEmpty(Fkmonedas))
            {
                ...
                Condiciones.Add(string.Format("{0}: {1}", RPedidos.Fkmonedas, serviceMonedas.getAll().Select(f=>(MonedasModel)f).SingleOrDefault(f => f.Id ==int.Parse(Fkmonedas)).Descripcion));
                flag = true;
            }
