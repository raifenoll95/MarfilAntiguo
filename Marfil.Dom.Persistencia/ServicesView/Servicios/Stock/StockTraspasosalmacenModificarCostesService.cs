...
{
    internal class StockTraspasosalmacenModificarCostesService : IStockService
    {
        ...
        public StockTraspasosalmacenModificarCostesService(IContextService context)
        {
            _context = context;
        }
        ...
        private Stockactual EditarPieza(MovimientosstockModel model)
        {
            var item = Db.Stockactual.Single(f =>
                        f.empresa == model.Empresa  && f.fkarticulos == model.Fkarticulos && f.lote == model.Lote &&
