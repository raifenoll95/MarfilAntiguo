...
{
    internal class OportunidadesConverterService : BaseConverterModel<OportunidadesModel, Oportunidades>
    {

        #region CTR

        public OportunidadesConverterService(IContextService context, MarfilEntities db) : base(context, db)
        {
        ...
        {
            var result = _db.Set<Oportunidades>().Where(f => f.empresa == Empresa).ToList().Select(f => GetModelView(f) as OportunidadesModel).ToList();
            return result;
        }
        ...
        public override bool Exists(string id)
        {
            ...
            return _db.Oportunidades.Any(f => f.empresa == Context.Empresa && f.id == intid);
        }

        public override IModelView CreateView(string id)
        {
            var identificador = Funciones.Qint(id);
            var obj = _db.Set<Oportunidades>().Where(f => f.empresa == Empresa && f.id == identificador).Single();
            var result = GetModelView(obj) as OportunidadesModel;                        
        ...
        public override Oportunidades CreatePersitance(IModelView obj)
        {
            var viewmodel = obj as OportunidadesModel;
            var result = _db.Set<Oportunidades>().Create();
        ...
        public override Oportunidades EditPersitance(IModelView obj)
        {
            var viewmodel = obj as OportunidadesModel;
            var result = _db.Oportunidades.Where(f => f.empresa == viewmodel.Empresa && f.id == viewmodel.Id && f.referencia == viewmodel.Referencia).Single();
        ...
        public override IModelView GetModelView(Oportunidades obj)
        {
            var result = base.GetModelView(obj) as OportunidadesModel;
            return result;
        }
