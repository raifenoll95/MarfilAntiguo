...
{
    internal class BuscarDocumentosService
    {
        private readonly MarfilEntities _db;
        private readonly string _empresa;

        public BuscarDocumentosService(MarfilEntities db,string empresa )
        {
            _db = db;
            _empresa = empresa;
        }
        ...
        public IEnumerable<DocumentosBusqueda> Buscar(IDocumentosFiltros filtros,out int registrostotales)
        {
            ...
            sbtotales.AppendFormat("Select count(*) from {0}  as d where d.empresa='{1}' {2}", tipo.ToString().Replace("Ventas",""), _empresa,GenerarFiltros(filtros));
            registrostotales = _db.Database.SqlQuery<int>(sbtotales.ToString()).Single();

            var sb = new StringBuilder();
            sb.AppendFormat("select d.*,e.descripcion as [Estado],m.decimales as [Decimalesmonedas]  from {0} as d left join monedas as m on m.id=d.fkmonedas left join estados as e on concat(e.documento,'-',e.id) = d.fkestados where d.empresa = '{1}' {3}", tipo.ToString().Replace("Ventas", ""), _empresa,(int)tipo,GenerarFiltros(filtros));
