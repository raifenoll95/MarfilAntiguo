...
{
    public class CuentasTiposProveedoresExclusiveApiController : ApiBaseController
    {
        public CuentasTiposProveedoresExclusiveApiController(IContextService context) : base(context)
        {
        }
        ...
        public HttpResponseMessage Get()
        {
            ...
            {
                var result = new ResultBusquedas<ProveedoresModel>()
                {
                    values = service.getAll().Select(f => (ProveedoresModel)f),
                    columns = new[]
                    {
        ...
        public HttpResponseMessage Get(string id)
        {
            ...
            using (var service = FService.Instance.GetService(typeof(ProveedoresModel),ContextService) as ProveedoresService)
            {
                var list = service.get(id) as ProveedoresModel;
                var response = Request.CreateResponse(HttpStatusCode.OK);
