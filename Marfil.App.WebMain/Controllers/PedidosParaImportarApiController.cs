...
{
    public class PedidosParaImportarApiController : ApiBaseController
    {
        public PedidosParaImportarApiController(IContextService context) : base(context)
        {
        }
        ...
        public HttpResponseMessage Get()
        {
            ...
            using (var service = FService.Instance.GetService(typeof(PedidosModel),ContextService) as PedidosService)
            {
                ...
                //Todo EL: Crear un nuevo metodo en Pedidosservice que nos devuelva los Pedidos que se pueden importar, con un parametro de entrada del cliente
                var listPedidos = service.BuscarPedidosImportar(cliente);
                var estadosService=new EstadosService(ContextService);
                var estados = estadosService.GetStates(DocumentoEstado.PedidosVentas);

                var result = new ResultBusquedas<PedidosModel>()
                {
                    values = string.IsNullOrEmpty(cliente)? listPedidos : listPedidos.Where(f=>f.Fkclientes==cliente),
                    columns = new[]
                    {
