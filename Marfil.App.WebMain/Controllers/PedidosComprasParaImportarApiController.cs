...
{
    public class PedidosComprasParaImportarApiController : ApiBaseController
    {
        public PedidosComprasParaImportarApiController(IContextService context) : base(context)
        {
        }
        ...
        public HttpResponseMessage Get()
        {
            ...
            using (var service = FService.Instance.GetService(typeof(PedidosComprasModel),ContextService) as PedidosComprasService)
            {
                ...
                var listPedidos = service.BuscarPedidosComprasImportar(cliente);
                var estadosService=new EstadosService(ContextService);
                var estados = estadosService.GetStates(DocumentoEstado.PedidosCompras);

                var result = new ResultBusquedas<PedidosComprasModel>()
                {
                    values = string.IsNullOrEmpty(cliente)? listPedidos : listPedidos.Where(f=>f.Fkproveedores==cliente),
                    columns = new[]
                    {
