...
{
    public class PresupuestosComprasLinParaImportarApiController : ApiBaseController
    {
        public PresupuestosComprasLinParaImportarApiController(IContextService context) : base(context)
        {
        }
        ...
        public HttpResponseMessage Get()
        {
            ...
            using (var service = FService.Instance.GetService(typeof(PresupuestosComprasModel),ContextService) as PresupuestosComprasService)
            {
                ...
                {
                    var serieslist = seriesService.GetSeriesTipoDocumento(TipoDocumento.PresupuestosCompras).Select(f=>new ComboListItem() {label =f.Id,value=f.Id}).ToList();
                ...
                var result = new ResultBusquedas<PresupuestosComprasLinImportarModel>()
                {
                    values = service.BuscarLineasPresupuestosCompras(cliente, presupuestosdesde, presupuestoshasta),
                    columns = new[]
                    {
                        new ColumnDefinition() { field = "FkPresupuestosComprasreferencia", displayName = "Presupuesto", visible = true ,width=150},
