...
{
    public class BancosMandatosApiController : ApiBaseController
    {
        public BancosMandatosApiController(IContextService context) : base(context)
        {
        }
        ...
        public HttpResponseMessage Get(string id)
        {
            ...
            using (var service = FService.Instance.GetService(typeof(BancosMandatosLinModel),ContextService) as BancosMandatosService)
            {
                var nvc = HttpUtility.ParseQueryString(Request.RequestUri.Query);
                var cuenta = nvc["fkcuenta"];
                var result = service.GetBancosMandatos(cuenta).Single(f=>f.Id == id);
                var response = Request.CreateResponse(HttpStatusCode.OK);
